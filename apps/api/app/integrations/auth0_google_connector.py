...
class Auth0GoogleConnector:
    ...
    def google_identity(self, user_sub: str) -> dict | None:
    ...
    def google_status(self, user_sub: str) -> dict:
        identity = self.google_identity(user_sub)
        if not identity:
            return {"linked": False, "provider": "google-oauth2", "provider_user_id": None, "has_access_token": False}
    ...
    def google_access_token(self, user_sub: str) -> str | None:
        identity = self.google_identity(user_sub)
        if not identity:
            return None

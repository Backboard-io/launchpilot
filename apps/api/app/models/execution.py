...
# Task category for gamification points: infra=0, coding=0, text_social=1, video_social=3
TASK_CATEGORY_POINTS = {"infra": 0, "coding": 0, "text_social": 1, "video_social": 3}


class LaunchPlan(Base, ProjectIdMixin, UUIDPrimaryKeyMixin):
    __tablename__ = "launch_plans"

    project_id: Mapped[str] = mapped_column(String, ForeignKey("projects.id", ondelete="CASCADE"), nullable=False)
    positioning_version_id: Mapped[str | None] = mapped_column(String, ForeignKey("positioning_versions.id"))
...
class LaunchTask(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    launch_plan_id: Mapped[str] = mapped_column(String, ForeignKey("launch_plans.id", ondelete="CASCADE"), nullable=False)
    ...
    category: Mapped[str | None] = mapped_column(String)  # infra | coding | text_social | video_social
    assignee_id: Mapped[str | None] = mapped_column(String, ForeignKey("users.id", ondelete="SET NULL"))


class Asset(Base, ProjectIdMixin, UUIDPrimaryKeyMixin, TimestampMixin):
...
class Contact(Base, ProjectIdMixin, UUIDPrimaryKeyMixin):
...
class OutboundBatch(Base, ProjectIdMixin, UUIDPrimaryKeyMixin):
...
class OutboundMessage(Base, UUIDPrimaryKeyMixin):
    ...
    batch_id: Mapped[str] = mapped_column(String, ForeignKey("outbound_batches.id", ondelete="CASCADE"), nullable=False)
    contact_id: Mapped[str] = mapped_column(String, ForeignKey("contacts.id", ondelete="CASCADE"), nullable=False)
